...
public class OrderService {
     ...
     *
     * 현재 코드의 문제:
     * 현재 코드에서는 트랜잭션이 시작되고 나서 오랜 시간동안 트랜잭션을 점유하고 있기 때문에 커넥션 읽기 타임아웃이 발생할 수 있습니다.
     * 또한 중간 진행률 저장을 위해 새로운 트랜잭션을 시작하여 업데이트 하고 있어 커넥션 풀이 고갈될 위험이 있습니다.
     *
     * 개선 방안
     * 이를 개선하기 위해서 bulkShipOrdersParent() 일괄 배송 처리하는 클래스 (퍼사드)를 만든 뒤,
     * 비즈니스 로직을 해당 퍼사드에서 실행합니다.
     * 1. 해당 Job 시작 (트랜잭션, 별도 클래스 위임)
     * 2. 주문 진행률 갱신 로직 시작
     *  2-1. 오래걸리는 작업
     *  2-2. 업데이트 (트랜잭션, 별도 클래스 위임)
     * 3. 해당 Job 완료 (트랜잭션, 별도 클래스 위임)
     *
     * 3번에서 예외가 나는 경우를 대비하여 별도 스케줄링을 통해 진행 상태를 확인합니다.
     *
     */
    @Transactional
